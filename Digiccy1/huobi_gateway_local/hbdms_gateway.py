...
class HbdmSwapGateway(BaseGateway):
    """
    ...
    def __init__(self, event_engine):
        """Constructor"""
        super().__init__(event_engine, "HBDMS")
...
class HbdmRestApi(RestClient):
    ...
    def on_query_position(self, data, request):
        """"""
        if self.check_error(data, "查询swap持仓"):
            return
    ...
    def on_query_contract(self, data, request):  # type: (dict, Request)->None
        """"""
        if self.check_error(data, "查询swap合约"):
            return
        ...
        self.gateway.write_log("swap合约信息查询成功")
    ...
    def on_send_order(self, data, request):
        ...
        if self.check_error(data, "swap委托"):
