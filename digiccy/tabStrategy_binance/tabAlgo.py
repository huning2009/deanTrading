...
class TabAlgoTemplate(object):
    ...
    def __init__(self, algoEngine, spread):
        ...
        self.mode = self.MODE_LONGONLY     # 工作模式
    ...
    def setAlgoParams(self, d):
        ...
        self.mode = d.get('mode', self.MODE_LONGONLY)
...
class TabSniperAlgo(TabAlgoTemplate):
    ...
    def quoteActiveLeg(self, direction):
        ...
        if direction == self.SPREAD_LONG:
            spreadVolume = min(spread.askVolume, self.maxOrderSize)
            
        else:
            spreadVolume = min(spread.bidVolume, self.maxOrderSize)
