...
EVENT_TIMER = 'eTimer' 
########################################################################
class TestApiStrategy(MmkTemplate):
    """基于布林通道的交易策略"""
    className = 'TestApiStrategy'
    ...
    def __init__(self, mmkEngine, setting):
        """Constructor"""
        super(TestApiStrategy, self).__init__(mmkEngine, setting)
    ...
    def onTick(self, tick):
        ...
        # print 'strategy onTick!%s' % tick.lastPrice
        if (tick.askPrice1 - tick.bidPrice1)/tick.bidPrice1 > self.spRatio:
            print 'spread large, bid:%s, ask:%s, spPerc:%s' % (tick.bidPrice1, tick.askPrice1, (tick.askPrice1 - tick.bidPrice1)/tick.bidPrice1)
    ...
    def onTimeFunc(self, event):
        # print 'strategy onTimeFunc is called'
        pass
