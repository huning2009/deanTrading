...
'''
本文件中实现了MMK策略引擎，针对MMK类型的策略，抽象简化了部分底层接口的功能。
'''
...
########################################################################
class MmkEngine(object):
    """MMK策略引擎"""
    settingFileName = 'MMK_setting.json'
    settingfilePath = os.path.join(os.path.dirname(os.path.abspath(__file__)), settingFileName)
    ...
    def __init__(self, mainEngine, eventEngine):
        ...
        # 注册日式事件类型
        self.mainEngine.registerLogEvent(EVENT_MMK_LOG)
    ...
    def sendOrder(self, vtSymbol, orderType, price, volume, strategy):
        ...
        req.volume = self.roundToContractSize(contract.size, volume)
        
        req.productClass = strategy.productClass
        req.currency = strategy.currency        
        
        # 设计为MMK引擎发出的委托只允许使用限价单
        req.priceType = PRICETYPE_LIMITPRICE    
        
        # MMK委托类型映射
        if orderType == MMKORDER_BUY:
            req.direction = DIRECTION_LONG
            req.offset = OFFSET_OPEN
            
        elif orderType == MMKORDER_SELL:
            req.direction = DIRECTION_SHORT
            req.offset = OFFSET_CLOSE
                
        elif orderType == MMKORDER_SHORT:
            req.direction = DIRECTION_SHORT
            req.offset = OFFSET_OPEN
            
        elif orderType == MMKORDER_COVER:
            req.direction = DIRECTION_LONG
            req.offset = OFFSET_CLOSE
        ...
        # self.writeMmkLog(u'策略%s发送委托，%s，%s，%s@%s' 
        #                  %(strategy.name, vtSymbol, req.direction, volume, price))
        
        return vtOrderIDList
    
    #----------------------------------------------------------------------
    def checkOrderStatus(self, vtSymbol, clientOrderID):
        contract = self.mainEngine.getContract(vtSymbol)
        gateway = self.mainEngine.getGateway(contract.gatewayName)
        print 'checkOrderStatus gateway:%s'% gateway
        gateway.tradeApi.getOrder(contract.symbol, origClientOrderId=clientOrderID)
    #----------------------------------------------------------------------
    def cancelOrder(self, vtOrderID):
        """撤单"""
        # 查询报单对象
        print 'mmkEngine cancelOrder:%s' % vtOrderID
    ...
    def processTickEvent(self, event):
        ...
        # print tick.vtSymbol, self.tickStrategyDict
        # 推送tick到对应的策略实例进行处理
        if tick.vtSymbol in self.tickStrategyDict:
            ...
            try:
                ...
                self.writeMmkLog(traceback.format_exc())
                return
                
            # 逐个推送到策略实例中
            l = self.tickStrategyDict[tick.vtSymbol]
            for strategy in l:
                # print self.tickStrategyDict, strategy.onTick
                self.callStrategyFunc(strategy, strategy.onTick, tick)
    
    #----------------------------------------------------------------------
    def processDepthEvent(self, event):
        """处理行情推送"""
        tick = event.dict_['data']
        newtick = copy.copy(tick)
        # 推送tick到对应的策略实例进行处理
        if tick.vtSymbol in self.tickStrategyDict:
            # 逐个推送到策略实例中
            l = self.tickStrategyDict[tick.vtSymbol]
            for strategy in l:
                # print self.tickStrategyDict, strategy.onTick
                self.callStrategyFunc(strategy, strategy.onDepth, newtick)
    #----------------------------------------------------------------------
    def processDetailEvent(self, event):
        """处理行情推送"""
        marketDetail = event.dict_['data']
        # if marketDetail.vtSymbol in self.tradeIDDict:
        #     if self.tradeIDDict[marketDetail.vtSymbol].detailID < marketDetail.detailID:
        #         print marketDetail.detailID
        #     else:
        #         print '!!!vtSymbol:%s, tradeID:%s, detailID:%s' % (marketDetail.vtSymbol, self.tradeIDDict[marketDetail.vtSymbol].detailID, marketDetail.detailID)
        #         print self.tradeIDDict[marketDetail.vtSymbol].__dict__
        #         print marketDetail.__dict__
        #     self.tradeIDDict[marketDetail.vtSymbol] = marketDetail
        # else:
        #     self.tradeIDDict[marketDetail.vtSymbol] = marketDetail

        newmarketDetail = copy.copy(marketDetail)
        # 推送tick到对应的策略实例进行处理
        if marketDetail.vtSymbol in self.tickStrategyDict:
            # 逐个推送到策略实例中
            l = self.tickStrategyDict[marketDetail.vtSymbol]
            # print 'processDetailEvent strategy count:%s, vtSymbol:%s' % (len(l), marketDetail.vtSymbol)
            for strategy in l:
                # print self.tickStrategyDict, strategy.onTick
                self.callStrategyFunc(strategy, strategy.onDetail, newmarketDetail)
    #----------------------------------------------------------------------
    ...
    #----------------------------------------------------------------------
    def processOrderStatusEvent(self, event):
        orderStatus = event.dict_['data']

        # 推送tick到对应的策略实例进行处理
        if orderStatus.vtSymbol in self.tickStrategyDict:
            # 逐个推送到策略实例中
            l = self.tickStrategyDict[marketDetail.vtSymbol]
            # print 'processDetailEvent strategy count:%s, vtSymbol:%s' % (len(l), marketDetail.vtSymbol)
            for strategy in l:
                # print self.tickStrategyDict, strategy.onTick
                self.callStrategyFunc(strategy, strategy.onOrderStatus, orderStatus)
        pass
    #----------------------------------------------------------------------
    ...
    def processPosEvent(self, event):
        ...
        if pos.symbol in self.posStrategyNameDict:
            for strateName in self.posStrategyNameDict[pos.symbol]:
                strategy = self.strategyDict[strateName]
                self.callStrategyFunc(strategy, strategy.onPos, pos)

    #----------------------------------------------------------------------
    def registerEvent(self):
        """注册事件监听"""
        self.eventEngine.register(EVENT_TICK, self.processTickEvent)
        self.eventEngine.register(EVENT_DEPTH, self.processDepthEvent)
        self.eventEngine.register(EVENT_DETAIL, self.processDetailEvent)
        self.eventEngine.register(EVENT_ORDER, self.processOrderEvent)
        self.eventEngine.register(EVENT_ORDERSTATUS, self.processOrderStatusEvent)
    ...
    def writeMmkLog(self, content):
        ...
        log.gatewayName = 'MMK_STRATEGY'
        event = Event(type_=EVENT_MMK_LOG)
    ...
    def loadStrategy(self, setting):
        ...
        if name in self.strategyDict:
            ...
            # print 'posStrategyNameDict:%s' % self.posStrategyNameDict
    #----------------------------------------------------------------------
    ...
    def initStrategy(self, name):
        ...
        if name in self.strategyDict:
            ...
            if not strategy.inited:
                ...
                self.writeMmkLog(u'请勿重复初始化策略实例：%s' %name)
        else:
            self.writeMmkLog(u'策略实例不存在：%s' %name)        
    ...
    def getStrategyVar(self, name):
        ...
        if name in self.strategyDict:
            ...
            self.writeMmkLog(u'策略实例不存在：' + name)    
            return None
    ...
    def putStrategyEvent(self, name):
        ...
        event = Event(EVENT_MMK_STRATEGY+name)
        event.dict_['data'] = d
        self.eventEngine.put(event)
        
        d2 = {k:str(v) for k,v in d.items()}
        d2['name'] = name
        event2 = Event(EVENT_MMK_STRATEGY)
    ...
    #----------------------------------------------------------------------
    def roundToContractSize(self, contractSize, volume):
        """取整价格到合约最小价格变动"""
        if not contractSize:
            return volume
        
        newVol = round(volume/contractSize, 0) * contractSize
        return newVol    
